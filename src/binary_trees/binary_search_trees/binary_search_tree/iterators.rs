...
macro_rules! impl_iter {
    ($iter: ident) => {
        paste! {
            ...
            impl<'t, K, V> TreeIterator<BstNode<K, V>> for [<$iter:camel>]<'t, K, V> {
                fn next_with_subtree_filter<P>(&mut self, predicate: P) -> Option<Self::Item>
                where 
                    P: FnMut(&BstNode<K, V>) -> bool
                {
                    self.0.next_with_subtree_filter(predicate).map(BstNode::data)
                }
            }
            ...
            impl<'t, K, V> TreeIterator<BstNode<K, V>> for [<$iter:camel Mut>]<'t, K, V> {
                fn next_with_subtree_filter<P>(&mut self, predicate: P) -> Option<Self::Item>
                where 
                    P: FnMut(&BstNode<K, V>) -> bool
                {
                    self.0.next_with_subtree_filter(predicate).map(BstNode::data_with_mut_value)
                }
            }
            ...
            impl<K, V> TreeIterator<BstNode<K, V>> for [<Into $iter:camel>]<K, V> {
                fn next_with_subtree_filter<P>(&mut self, predicate: P) -> Option<Self::Item>
                where 
                    P: FnMut(&BstNode<K, V>) -> bool
                {
